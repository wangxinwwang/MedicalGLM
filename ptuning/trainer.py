...
class Trainer:
    ...
    def compute_strategy_loss(self, generated_responses, device,outputs):
        quality_scores = [self.compute_quality_score(response, device) for response in generated_responses]
        quality_scores_tensor = torch.tensor(quality_scores, device=device)
        log_probs=torch.log_softmax(outputs.logits,dim=-1)
        strategy_loss = - 0.1*torch.mean(log_probs*quality_scores_tensor)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.args.n_gpu > 1:
            original_loss = original_loss.mean()  # mean() to average on multi-gpu parallel training

        if self.args.gradient_accumulation_steps > 1 and not self.deepspeed:
            original_loss = original_loss / self.args.gradient_accumulation_steps

        # Use already generated responses to compute strategy loss
        with torch.no_grad():
            generated_responses = self.tokenizer.batch_decode(inputs['input_ids'], skip_special_tokens=True)
            strategy_loss = self.compute_strategy_loss(generated_responses, device)

        # Combine original loss with strategy loss
        combined_loss = original_loss + 0.1 * strategy_loss  # Adjust the weight of strategy loss

        if self.do_grad_scaling:
            self.scaler.scale(combined_loss).backward()
        elif self.use_apex:
            with amp.scale_loss(combined_loss, self.optimizer) as scaled_loss:
                scaled_loss.backward()
        elif self.deepspeed:
            combined_loss = self.deepspeed.backward(combined_loss)
        else:
